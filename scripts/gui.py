...
- 下方提供云主机复选框，用户可自行勾选需要的云主机（包括管理节点）。
- "全选云主机" / "取消云主机" 快捷按钮。
- 实时汇总展示月/年/折扣价等信息。
...
class QuoteApp:
    ...
    def refresh_summary(self):
        # 计算费用，仅针对已选产品（不自动加入关联主机）
        data = []
        # 复制已选产品列表，后续若勾选云主机会在 generate 时加入
    ...
    def generate_quote(self):
        if not self.selected_products:
            ...
            return
        # 合并选中的云主机
        selected = list(self.selected_products)  # copy
        # 添加手动勾选的云主机
