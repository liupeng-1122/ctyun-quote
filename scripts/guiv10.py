# -*- coding: utf-8 -*-
"""
天翼云等保专区报价工具 - GUI 交互界面（版本10）
修复问题：
1. 云主机选项：恢复独立的"云主机"分类，同时将云主机放在对应安全产品目录下
2. 下一代防火墙：确认三个版本均已包含（标准版id14、高级版id15、企业版id17）
3. 报价表表头：标注对应报价年数（通过 main.py 的 create_quote_excel 支持）
"""
...
class QuoteApp:
    def __init__(self, root):
        self.root = root
        self.root.title("天翼云等保专区报价工具（V10）")
    ...
    def populate_product_tree(self):
        ...
        for cat, items in categories.items():
            ...
            for prod in items:
                ...
                # 如果产品需要云主机，将云主机作为子节点放在对应目录下（唯一iid避免冲突）
                if prod.get('require_host') and prod.get('host_id'):
                    ...
                    if host:
                        ...
                        if not self.product_tree.exists(host_iid):
                            self.product_tree.insert(cat_id, 'end', iid=host_iid, text=f"[云主机] {host['name']}")
        # 不再添加独立的云主机根节点，云主机仅作为对应安全产品的子目录显示
    ...
    def on_product_double_click(self, event):
        ...
        if item.startswith('p'):
            ...
            parts = item.split('_')
            host_part = parts[0]  # e.g., h12
            host_id = int(host_part[1:])
    ...
    def generate_quote(self):
        ...
        if not out_path:
            return
        create_quote_excel(data, totals, out_path, f"天翼云等保专区安全产品报价表（自定义，{years}年）", years=years)
