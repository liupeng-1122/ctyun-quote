...
class QuoteApp:
    def __init__(self, root):
        self.root = root
        self.root.title("天翼云等保专区报价工具（V6）")
    ...
    def generate_quote(self):
        if not self.selected_products:
            ...
            return
        years = self.year_var.get()
        # 使用原始 generate_quote_data 生成基础数据（不含年份），随后在写入 Excel 前按年份调整金额
        data = generate_quote_data(self.selected_products, self.quantities, include_hosts=False, include_mgmt_host=False)
        # 调整每行的金额字段
        for row in data:
            # row 中的 yearly、discounted、price_45、price_55 均为 1 年的值，按 years 放大
